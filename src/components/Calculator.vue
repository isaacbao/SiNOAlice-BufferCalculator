...
<script>
  ...
  Vue.component('active-skill-selector', {
    props: ['activeSkills'],
    template: '<li>{{ todo.text }}</li>'
  });

  Vue.component('active-skill-option', {
    props: ['activeSkill'],
    template: '<option value={{activeSkills.id}}>{{ activeSkill.name+(+activeSkill.description+) }}</option>'
  });
